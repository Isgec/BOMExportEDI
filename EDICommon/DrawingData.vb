...
Public Class DrawingData
  ...
  Public Shared Sub WriteXMLDocument(ByVal xData As DrawingData, ByVal Job As jobFile, ByVal xmlPath As String, ByVal ORGFilePathName As String)
    ...
    With xData
      Try
        ...
        tmpA = document.CreateAttribute("slzFileName")
        tmpA.Value = xmlPath & "\" & IO.Path.ChangeExtension(xData.DrawingFileName, ".slz")
        tmpN.Attributes.Append(tmpA)

        tmpA = document.CreateAttribute("ORGFilePathName")
        tmpA.Value = ORGFilePathName & "\" & xData.DrawingFileName
        tmpN.Attributes.Append(tmpA)
        ...
        For Each refDoc As RefDwgData In .RefDwgs
          ...
          tmpA = document.CreateAttribute("filename")
          If refDoc.FileName = "" Then
            tmpA.Value = (refDoc.DrawingID & ".dwg")
          Else
            tmpA.Value = (refDoc.FileName)
          End If
          rdocNode.Attributes.Append(tmpA)
      ...
      End Try
    End With
    document.Save(xmlPath & "\" & IO.Path.ChangeExtension(xData.DrawingFileName, ".xml"))
    'DrawingData.Save(xmlPath & "\" & IO.Path.ChangeExtension(xData.DrawingFileName, ".slz"), xData)
  End Sub

  Public Shared Sub Save(ByVal FileName As String, ByVal oApl As DrawingData)
    Dim oSrz As XmlSerializer = New XmlSerializer(GetType(DrawingData))
  ...
  Public Shared Function Open(ByVal FileName As String) As DrawingData
    If IO.File.Exists(FileName) Then
      Dim oApl As DrawingData = Nothing
